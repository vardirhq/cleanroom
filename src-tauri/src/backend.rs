...
pub fn get_app_bootstrap_sync() -> AppBootstrap {
    ...
    AppBootstrap {
        ...
        scan_summary: package_snapshot.summary.clone(),
...
pub fn set_active_device_sync(serial: Option<String>) {
    set_active_device_serial(serial);
}

pub fn update_app_settings_sync(
    adb_strategy: String,
    developer_mode: bool,
...
pub fn get_device_signature_sync() -> String {
    let adb = AdbRunner::detect();
    load_device_signature(adb.as_ref())
}

pub fn get_package_metadata_sync(package_names: Vec<String>) -> Vec<PackageMetadataRecord> {
    let adb = AdbRunner::detect();
    let active_serial = get_active_device_serial();
    let device_snapshot = load_device_snapshot(adb.as_ref(), active_serial.as_deref());
...
pub fn get_cleanup_report_sync(report_id: String) -> Option<CleanupSessionReport> {
    load_cleanup_report(&report_id)
}

pub fn export_cleanup_report_pdf_sync(
    report_id: String,
    language: PdfExportLanguage,
) -> PdfExportResult {
    export_pdf_report(&report_id, language)
}

pub fn open_launcher_recovery_settings_sync(target: String) -> Result<String, String> {
    let adb =
        AdbRunner::detect().ok_or_else(|| "ADB is not available on this system.".to_string())?;
    let active_serial = get_active_device_serial();
